...
*Zero Trust Intelligence Protocol — Why It Exists and Where It's Going*
...
ZTIP exists so that organizations do not have to choose.
...
ZTIP defines how every agent interaction is wrapped in a structured, verifiable artifact — one
...
ZTIP replaces implicit trust with an explicit, policy-evaluated, artifact-backed transaction.
...
ZTIP's hash-linked artifact structure produces an unambiguous, tamper-evident record of every
...
ZTIP governs the transaction, not the toolchain. Any agent framework can participate in ZTIP by
...
ZTIP formalizes policy-driven authorization as a structured record embedded in the transaction
...
ZTIP is transport-agnostic. The artifact travels however it needs to travel. The governance
travels with it.
...
ZTIP does not constrain which agents, models, frameworks, or tools engineers use. It constrains
whether a transaction between them is authorized.

Engineers choose their tools. The organization defines its policies. ZTIP is the layer that
...
ZTIP defines the artifact structure that makes any transaction verifiable, regardless of how it
moves.
...
the protocol itself. ZTIP supports the full range of policy outcomes:
...
- **Human approval required** — organizational policy requires a named individual or role to
  approve before the transaction proceeds. ZTIP supports optional human-in-the-loop approval
...
ZTIP artifacts are plain JSON. They are human-readable, debuggable, storable in any log system,
...
deployment contexts, but it is not ZTIP's responsibility to require it.

### Auditability is not optional

A transaction that cannot be audited is a transaction that cannot be governed. ZTIP's artifact
...
If a ZTIP artifact cannot answer all three questions, it is not a complete artifact.

### The control plane is authoritative

ZTIP does not self-govern. Authorization records are issued by a compliant control plane —
...
## The Relationship Between ZTI, ZTIP, and ZTI Core
...
**ZTIP — the open protocol**
ZTIP is the agent-level expression of the ZTI doctrine. It defines how governed transactions
between agents — and between agents and humans or systems — must be structured. ZTIP is
transport-agnostic and framework-agnostic. It is an open protocol, not tied to any specific
implementation.

**ZTI Core — the control plane**
ZTI Core is a compliant implementation of the control plane that the ZTIP protocol requires.
It evaluates policy, issues authorization records, stores artifacts, and provides audit access.
Organizations may use ZTI Core or build their own control plane that satisfies ZTIP's
authorization and auditability requirements.

```text
ZTI (verification doctrine)
  └── ZTIP (open transaction protocol — this repo)
        └── ZTI Core (one compliant control-plane implementation)
```

ZTIP defines what must be true of a governed transaction. ZTI Core is one way to make it true.

---

## How ZTIP Ensures Integrity and Auditability

Each ZTIP transaction produces a structured artifact containing:
...
The first version of ZTIP is scoped to a single-organization deployment:
...
workloads today have zero transaction governance. ZTIP's initial scope delivers governance where
...
ZTIP publishes a canonical schema, validation rules, and a reference artifact library. Any agent
framework can produce a valid ZTIP artifact with a thin integration layer. Compliance can be
verified programmatically.

### Ecosystem integration

Agent orchestration platforms, IDE plugins, and CI/CD tools adopt ZTIP artifact production
...
agents interact with another's — ZTIP provides the trust artifact that makes those interactions
...
ZTIP's doctrine and specification groundwork is delivered:
...
4. **Reference implementation** — the `ztip/` runtime: a minimal library and CLI that
   canonicalizes, hashes, and verifies ZTIP artifacts, independent of any agent framework.
...
> ZTIP is the open protocol for governed agent transactions.
